class ShowThreadUseCase {
    ...
    async execute(useCasePayload) {
      ...
      const validatedComments = this.validateDeletedComment(comments);
      const validatedReplies = this.validateDeletedReply(replies);
      const commentsAndReplies = this.addReplyToComment(validatedComments, validatedReplies);
    }
  

    _validateDeletedComment(comments) {
      for (const comment of comments) {
        if (comment.is_delete) {comment.content = 'coment dihapus'}
    ...
    _validateDeletedReply(replies) {
      for (const reply of replies) {
        if (reply.is_delete) {reply.content = 'balasan dihapus'}
